...
**Features completadas**: 106/126 (`feature_list.json`)
**En progreso**: ninguna en esta rama. #124 `mobile-home-bell-icon-source-lock-unbounded` cerrada con reviewer aprobado; pendiente del merge humano de su PR

**Pendientes**: 20 (#18, #41, #60, #74, #77, #80, #81, #86, #99-#101, #103, #105, #115-#120 y #126). **#124 `mobile-home-bell-icon-source-lock-unbounded` cerrada**: la asercion del icono de la campana de la Home (`expect(source).toContain('<Bell size={24} color={muted} />')`) miraba TODO `src/screens/home/index.tsx`, asi que una copia senuelo `{false && <Bell size={24} color={muted} />}` le prestaba el verde con la campana en `accent`. Ahora un `describe` anidado en `#78 R10` asevera el color en el arbol, sin y con alertas abiertas, con `Uniwind.getCSSVariable` espiado y el literal `'--color-muted'`; la linea `toContain` se queda con un comentario que explica por que. Diff de produccion **vacio**; el rojo versiono la mutacion B2 y el verde la revirtio. 21/21 sondas con el veredicto exigido, re-medidas todas por el reviewer; el residuo B5d/B6d queda como limite documentado por decision del humano. Reviewer aprobado a la primera. **#126 registrada** al cerrarla: `#69 R9` cuenta los iconos de las cuatro celdas sobre todo `index.tsx` y la misma copia senuelo le presta el verde (sonda W2, latente). **#125 `reminder-advance-already-past` cerrada** (decision del humano C + A): en Nuevo recordatorio los chips de aviso cuyo momento ya paso quedan desactivados y la seleccion es el mayor aviso activo que no supera el ultimo elegido, asi que un recordatorio a menos de 7 dias ya no dispara el push al guardar; y el cuerpo del push dice cuando vence (`Recordatorio: <titulo> · 1 de octubre a las 09:00`) en la zona del owner, via el `findOwnerTimezone` que ya existia. Reviewer aprobado a la primera; su H1 (ningun test distinguia conservar la eleccion de resetear a 7 dias) se cerro con la Enmienda E1 (R6, solo tests), firmada en Notion y aprobada en la ronda 2. Smoke en dev build firmado por el humano. **#122 `mobile-source-lock-slice-blind-spots` cerrada**: los tres candados de fuente que recortan el tag de apertura de `<` a `<` (el `meal-toggle` de Nutricion, la campana y el enlace «Ver todos» de la Home) tenian verdes falsos en zona ciega: un segundo tag con la misma ancla (ternario o senuelo `{false && ...}`) les prestaba la receta, y un comentario JSX con la receta antes del primer hijo, o un spread detras de ella, dejaba el propio tag en `opacity: 1` con la suite en verde. Dos defensas, solo de test: R1 asevera que el ancla es unica (`lastIndexOf` igual a `indexOf`) y R2 anade tres `it` que pulsan el elemento con `responderGrant` y leen la opacidad renderizada (`toHaveStyle({ opacity: 0.8 })`); `pressIn` no llega a nadie y `userEvent.press` suelta antes de leer. Diff de produccion **vacio**; los dos rojos versionaron su mutacion de produccion y los verdes la revirtieron. 78/78 sondas con el veredicto exigido; O2c (spread condicional al estado) sigue verde y queda como limite documentado en `docs/conventions.md`. Reviewer aprobado a la primera. La redundancia de la pata de fuente de S y M con R2 (hallazgo F) no se registra: la unicidad vive en ese mismo `it` y en la campana la regex es el unico control del reposo. **#123 `mobile-date-picker-utc-day-shift` cerrada** (bug P1): el dialogo de fecha de Android de `@expo/ui` devuelve el dia elegido a medianoche UTC y abre tomando el valor como dia UTC, asi que en Mexico (UTC-6) Nuevo recordatorio y la fecha de nacimiento de Anadir mascota guardaban el dia anterior y, de noche, el calendario abria en manana. `src/utils/date-picker-value.ts` (`toPickerValue` / `fromPickerValue`) convierte en las dos direcciones solo en Android (iOS devuelve instante local, D2); `minimumDate`, `maximumDate` y el picker de hora no se tocan. Reviewer aprobado a la primera, con la suite corrida en 7 zonas reales. Smoke en dev build firmado por el humano. **#125 `reminder-advance-already-past` registrada** (P2, desde el smoke de #123): el aviso por defecto «7 dias antes» dispara el push al guardar si faltan menos de 7 dias, y el cuerpo no dice cuando vence; el humano eligio desactivar en la app los avisos que ya pasaron y decir la fecha en el push. **#121 `mobile-home-bell-source-lock-unbounded` cerrada**: el candado de la receta de pulsado de la campana de la Home (`#78 R10`) buscaba la receta en TODO `src/screens/home/index.tsx`, y la copia de `reminders-see-all` le prestaba el verde: con `style={{ opacity: 1 }}` en la campana quedaba verde la suite movil entera. Ahora asevera contra el tag de apertura propio de `home-alerts-bell`, de `<` a `<`, el patron de #109 y #112; las otras tres aserciones del `it` (ruta, cast, icono) no cambian. Diff de produccion **vacio**; `docs/conventions.md` lista la campana como tercer call-site y anade que aseverar una receta contra el fichero entero no vigila el elemento si otro la repite. El sandbox de Codex denego `rm -f .expo/types/router.d.ts` y Codex paro, como pedia el handoff; el humano autorizo sustituirlo por `test ! -e` (Enmienda 1), sin borrar nada. Reviewer aprobado a la primera. **#124 registrada** al cerrarla: la asercion del icono `<Bell size={24} color={muted} />` del mismo `it` tambien mira el fichero entero (latente: necesita una segunda copia literal). **#122 enmendada**: tres call-sites y tres hallazgos del veredicto (O2: un spread `{...override}` detras de la receta engana a cualquier candado de fuente y `tsc` no lo para; O5: el ancla no unica tambien con codigo renderizado; O4: rojo falso seguro por la misma raiz). **#84 `reminder-dates-days-until-drift` cerrada**: `daysUntil` de la pantalla de Recordatorios contaba bloques de 24 h redondeados hacia arriba (hoy a las 20:00 salia «en 1 dias», +7 dias a hora posterior quedaba fuera de «Esta semana» y ayer a hora posterior daba `-0` y llevaba «¡Proximo!»); ahora resta el dia civil local con dos `Date.UTC`, misma firma, sin tocar la pantalla. La ronda 1 se rechazo por la spec, no por Codex: todas las fechas de sus tablas caian del 9 al 20 de septiembre y `to.getDate() - from.getDate()` dejaba verde la suite entera. La Enmienda E1 (firmada en Notion) anadio R5 (fin de mes, fin de ano y Nochevieja en Ciudad de Mexico), R6 (umbrales 7 y 10) y paso las filas heredadas a componentes locales. Reviewer aprobado en la ronda 2; smoke en dev build de Android superado por el humano. **#123 `mobile-date-picker-utc-day-shift` registrada** (2026-09-24, sesion Backend, bug reportado por el humano en el smoke de #84): el calendario nativo de Android de `@expo/ui` devuelve el dia elegido a medianoche UTC y la app lo lee en local, asi que en Mexico (UTC-6) Nuevo recordatorio y la fecha de nacimiento de Anadir mascota guardan el dia anterior. P1: por orden del humano va justo despues de cerrar #84. **#112 `mobile-reminders-see-all-source-lock-nesting` cerrada**: el candado de fuente del enlace «Ver todos» de la Home recortaba de `<Pressable` a `</Pressable>`, asi que un Pressable anidado le prestaba su style y el candado pasaba en VERDE vigilando el elemento equivocado. Ahora recorta de `<` a `<`, el tag de apertura propio, como el `meal-toggle` de #109. Diff de produccion **vacio**; `docs/conventions.md` ya no cita numeros de linea caducados y dice que no queda ningun recorte `<Tag`→`</Tag>` por migrar. La mutacion versionada no fue la de #109: aqui el mismo `it` asevera primero el arbol, que ya ve la opacidad en reposo, asi que el agujero solo existia en el estado pulsado. Reviewer aprobado a la primera. **#120-#122 registradas** al cerrarla: `elementWithTestId` recorta con los hijos dentro (#120), el candado de la campana `#78 R10` busca la receta en todo `index.tsx` (#121) y tres verdes falsos en zona ciega del recorte de `<` a `<` (#122: comentario JSX antes del primer hijo, receta comentada, ancla no unica). **#114 `mobile-reminders-alerts-to-stack` cerrada**: `reminders` y `alerts` salen de `(tabs)` al `Stack.Protected` de #95, con cabecera nativa y sin barra de pestanas, por decision de producto del humano (P1 = A, firmada en Notion). Tocar una notificacion ya no apila una segunda `(tabs)`, y `alerts` es `dangerouslySingular`. `add-reminder` sin mascota hace `dismissTo('/reminders')` en vez de `<Redirect>`. Cero claves de catalogo. Reviewer aprobado a la primera; su Obs. 1 (candado tautologico del segundo toque en el test de R3) queda anotada en #100. **#115-#119 registradas** (2026-09-23, sesion Backend, a peticion del humano) desde una auditoria de paridad con el Figma Make: Salud con grafica de peso, cabecera y dias restantes (#115), pildora GPS y bateria en el Mapa (#116), Recuperar contrasena activa (#117, la deuda `mobile-forgot-password` que nunca tuvo id), bienvenida inicial (#118) y cabecera fotografica en Nutricion y, si el humano enmienda A11/A13, en las pantallas de detalle (#119). Son de UI y, por orden del humano, van AL FINAL: no se especifican hasta cerrar las pending anteriores. La auditoria: de 17 pantallas del Make, 3 con paridad alta, 7 media, 5 baja y 2 ausentes. **#113 `mobile-kcal-consumed-bar` cerrada**: la tarjeta Objetivo diario de Nutricion pinta las kcal servidas hoy contra `merKcal` con el `kcalConsumedToday` de #104: una barra a todo el ancho bajo la fila, `{n} kcal` y `{pct}%`, relleno que transiciona su ancho en 250 ms con la receta de #106, un unico `progressbar` accesible con la clave nueva `food.kcalConsumedOfTarget` y el esqueleto a `h-40`. D1, firmada por el humano: barra sola, sin el anillo del Make, que repetia el mismo porcentaje y obligaba a enmendar `mobile-food`. La ronda 1 se rechazo por un candado que no candaba: `toHaveAnimatedStyle` solo compara las claves que se le pasan, y una opacidad o un relleno del color de la tarjeta (barra invisible) pasaban en verde. Lo cerro R7 en la Enmienda E1, firmada en Notion, con `shouldMatchAllProps` y `style` candado en los nodos no-texto; su rojo fue una mutacion de produccion versionada y revertida. Queda H6 (baja): un estilo condicionado a proposito al 100 % sobreviviria, porque R7 mira 0 % y 63 %; no se registra. Smoke en dev build de Android firmado por el humano en su propio commit. **#104 `nutrition-kcal-consumed` cerrada**: el `GET` del plan de nutricion devuelve `kcalConsumedToday`, las kcal de las franjas servidas hoy con reparto uniforme y un solo redondeo sobre el agregado (`Math.round(merKcal * servidas / mealsPerDay)`), derivado al leer con el plan vigente y el mismo dia civil del owner que `servedToday` de #83. Sin migracion ni cambios de puerto. Partida por D1, firmada por el humano: la tarjeta Objetivo diario es #113 `mobile-kcal-consumed-bar`, y OJO, el Make pinta barra Y anillo, no solo barra como decia la entrada de #104. Tercera spec aprobada desde Notion. **#110 `mobile-reanimated-double-dead-weight` cerrada**: el fichero de tests de la Home montaba dobles que no candaban nada -quitarlos dejaba la suite verde- pero sustituian el Skeleton real de heroui-native por un View pelado en sus 138 tests, borrando la clase base, el borderCurve y toda la superficie de animacion. Ahora se monta el real y hay dos tests nuevos que lo candan: 140 en el fichero, 1398 en la suite. OJO, la premisa con la que se registro la deuda era MAS ANCHA de lo que resulto: se dijo que el test de #62 R8 creia probar produccion y probaba un doble, y se midio que NO -mutar lo que vigila da 1 rojo identico con doble y sin doble-; sobrevive porque asevera con toContain y ambos Skeletons propagan className. Los titulos de sus tests se apartan a proposito de la forma canonica -llevan `R<n> (nombre-de-feature)` sin almohadilla- porque el literal `#110` casa con el guard de colores hex y pondria rojos los cinco guards que vigilan ese fichero; es desviacion firmada por el humano, no atajo. Segunda spec aprobada desde Notion. **#109 `mobile-meal-toggle-source-lock-nesting` cerrada**: el candado de fuente del boton de comida recortaba entre `lastIndexOf('<Pressable')` y `indexOf('</Pressable>')`, y un Pressable anidado dentro cortaba en el cierre del HIJO, dejando el tag de apertura del hijo con su style dentro del bloque -o sea que el candado pasaba en VERDE vigilando el boton equivocado, sin avisar. Ahora recorta de `<` a `<`, su propio tag de apertura, y el anidamiento deja de existir como concepto: dos indexOf, ningun simbolo nuevo que vigilar. Diff de produccion **vacio** y el patron queda escrito en `docs/conventions.md` con sus DOS limites conocidos y cual de los dos avisa. Primera spec aprobada desde Notion (#147): el humano movio `Estado del gate` y el leader firmo en el repo citando la pagina y su marca de tiempo. Esa primera vuelta destapo que la API da que y cuando pero **no la cuenta** que aprobo, y el harness ya lo dice asi. **#106 `mobile-meals-bar-motion` y #107 `mobile-meal-toggle-press-lock` cerradas y mergeadas**: la barra de comidas de la Home transiciona su ancho con `withTiming` respetando reduce motion, servir y deshacer vibran distinguiendo exito de fallo, y el feedback de pulsado del boton por franja por fin tiene candado. Son dos entradas cerradas con UNA sola spec y un solo ciclo, por decision del humano: tocaban el mismo Pressable. Entro `expo-haptics` ~57.0.3, la primera dependencia nueva desde el veto, firmada por el humano junto con la enmienda a `docs/ui-guidelines.md:171` que decia que no estaba instalada. Las tres rondas se fueron en candados que no candaban: B1 aseveraba contra la constante importada de produccion (mutar 250 a 2500 dejaba 138/138 verde) y B6 muestreaba la curva en dos puntos, agujero que el reviewer demostro construyendo con un solver una impostora que pasaba en verde desviandose 0.33 a mitad de recorrido. Los dos estan cerrados. Quedan #109 (agujero por anidamiento en el recorte del fuente de R5) y #110 (el doble de Reanimated sustituye el Skeleton real en 138 tests, incluido el de #62 R8, que cree probar produccion y prueba un doble). El guard de hex de `design-drift.test.ts` -todo R-id de tres cifras casa con `/#[\da-f]{3,8}\b/i` desde #100- se lo llevo la sesion Backend como #108. #94 `mobile-map-staleness-single-source` y #98 `mobile-meals-served-ui` están `done`; #98 ya está en `main` (PR #144) y la rama de #94 integra esa punta para que el PR #143 quede listo para el merge humano.
