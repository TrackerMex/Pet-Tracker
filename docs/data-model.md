...
| `devices` | id PK, esn UNIQUE, imei UNIQUE, serial_number UNIQUE, activation_code UNIQUE, wialon_unit_id UNIQUE, model, status CHECK ('available','assigned','inactive'), battery_pct, connectivity, last_message_at, ingest_watermark, is_simulated | Collares; seed simulado SIM-001..003. Los 5 identificadores UNIQUE (devices-claim D4): el claim busca por cualquiera de los 4 primeros y un match múltiple sería ambiguo. `status` es caché de presentación — la disponibilidad se deriva de la fila activa en `pet_devices` (D3) |
