...
| `email_verification_tokens` | id PK, user_id FK CASCADE, token_hash char(64) UNIQUE NOT NULL, expires_at timestamptz NOT NULL, used_at timestamptz NULL, created_at | Creada por `auth-registration` (#3). Solo se guarda el SHA-256 hex del token de un solo uso — el valor en claro nunca se persiste (ver `specs/auth-registration/design.md`). `used_at` NOT NULL = ya consumido, no reutilizable |
