...
| `OPENAI_ENABLED` | Activa la explicación IA solo con el valor literal `'true'`; cualquier otro valor, incluida su ausencia, degrada a `aiExplanation: null` | en `.env.example` (con `false`) — consumida desde `nutrition-ai-explainer` (#18): `src/modules/nutrition/infrastructure/ai/nutrition-explainer.factory.ts` vía `ConfigService` |
| `OPENAI_API_KEY` | Clave del proveedor; ausente, vacía o `PENDING` desactiva el adaptador real. Nunca se commitea una clave real | en `.env.example` (con `PENDING`) — consumida desde `nutrition-ai-explainer` (#18): `src/modules/nutrition/infrastructure/ai/nutrition-explainer.factory.ts` vía `ConfigService` |
| `OPENAI_MODEL` | Modelo usado para explicar el plan; no existe default dentro de `src/` | en `.env.example` — consumida desde `nutrition-ai-explainer` (#18): `src/modules/nutrition/infrastructure/ai/nutrition-explainer.factory.ts` vía `ConfigService` |
