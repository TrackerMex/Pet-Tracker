...
| R9 | `SEC`::`it('esqueletiza mientras carga y calla cuando el perfil falla')`: esqueleto con su `className` en `pending()`, cuerpo con **cero** hijos en `{ kind: 'error' }`, ningún `testID` de sección terminado en `-error` o `-retry`, y `reminders-see-all` presente en los dos escenarios + `SEC`::`it('deja el cuerpo con un solo hijo')`, trasladado íntegro por D1: cuenta **`getByTestId('reminders-section-body').children`** —nunca coincidencias de `testID`— y espera `1` cargado, `1` con esqueleto y `0` en error. Es el candado que mata M5 | pendiente |
