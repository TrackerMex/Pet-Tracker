...
| R2 | `SEC`::`it('tipa nextVaccine con los tres campos del contrato y ninguno más')`, lectura de `src/api/types.ts`: `id`/`name`/`nextDoseAt` presentes, `daysLeft` y `date` ausentes, `nextReminder` y `activitySummary` **todavía `unknown`**; más `tsc --noEmit` de R19, que es lo que prueba que el tipo compila contra el uso de R6. **Candado de cadena literal**, declarado como tal en [[requirements]] R17 | pendiente |
...
| R4 | `FMT`::`describe('#70 R4: calendarDaysUntil cuenta días de calendario')`::`it('cuenta futuro, mañana, hoy y pasado')`: `'2026-09-15'`→`5`, `'2026-09-11'`→`1`, `'2026-09-10'`→`0`, `'2026-09-08'`→`-2`, todos desde `new Date(2026, 8, 10, 12, 0)` | pendiente |
