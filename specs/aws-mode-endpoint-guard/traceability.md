...
| R4 | `backend-pet-tracker/src/aws/aws-endpoint-guard.spec.ts::R4: modo local intacto` + `backend-pet-tracker/src/aws/aws-mode.spec.ts` (adaptado, [[design]] §D10) | `backend-pet-tracker/src/aws/aws-clients.ts`, `backend-pet-tracker/src/aws/aws-mode.spec.ts` | regresión verde inicial: `c9027d2 test(aws-mode-endpoint-guard): preserve already-green local behavior (R4)`; sin cambio productivo, según `tasks.md` R4(2). Adaptación de los fixtures de #19: pendiente |
| R5 | `backend-pet-tracker/src/aws/aws-endpoint-guard.spec.ts::R5: la guarda cubre las dos vías de resolución` | `backend-pet-tracker/src/aws/aws-clients.ts` | rojo: `71d52b6 test(aws-mode-endpoint-guard): cover both config resolvers (R5)`; verde por registrar |
