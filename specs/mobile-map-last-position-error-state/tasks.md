...
- [x] (1) Añadir a `src/app/(tabs)/__tests__/map.test.tsx` (SOLO adiciones,
...
- [x] (2) En `src/app/(tabs)/map.tsx`, exactamente los cambios de [[design]]
      ...
      verdes
- [x] (3) Refactor con tests verdes — se espera que **no haya nada que
      refactorizar**; si aparece, es señal de que (2) creció de más

## R5 — Cierre y contención

- [x] (1) Rellenar `traceability.md` (archivo::describe::it + hashes de los
      dos commits por fila) y marcar las casillas de este archivo
- [x] (2) Ejecutar y registrar salida completa en
      `progress/impl_mobile-map-last-position-error-state.md`:
      `bun run test -- 'src/app/(tabs)/__tests__/map.test.tsx' --runInBand`,
...
- [x] (3) Avisar al leader: la feature queda lista para el `reviewer`. NO
