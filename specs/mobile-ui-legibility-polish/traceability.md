...
|---|---|---|---|---|
| R1 | AC8 — "El botón destructivo del bottom sheet de reminders resuelve su etiqueta con el token de danger, no con el del acento" | `src/__tests__/legibility-classnames.test.ts::#61 R1: la etiqueta destructiva usa el token de danger` | **21** | pendiente |
| R2 | AC1 — "Un token de texto sobre acento… da >=4,5:1 contra bg-accent en los dos temas; --accent conserva #2AB87C exacto y ningún relleno cambia de color" | `src/theme/__tests__/global-css.test.ts::#61 R2: token accent-contrast con AA sobre el relleno de acento` | **1** (token) | pendiente |
| R3 | AC1 — misma fila, mitad de uso: ningún relleno cambia, solo el color de la etiqueta | `src/__tests__/legibility-classnames.test.ts::#61 R3: todo texto sobre bg-accent usa text-accent-contrast sin opacidad` | **1** (17 sitios) | pendiente |
| R4 | AC2 — "text-accent como color de enlace sobre bg-surface y sobre bg-default pasa AA" | `src/theme/__tests__/global-css.test.ts::#61 R4: token accent-strong con AA sobre superficies neutras` + `src/__tests__/legibility-classnames.test.ts::#61 R4: los enlaces y valores de acento usan text-accent-strong` | **3** | pendiente |
...
Los diez criterios de `feature_list.json` quedan cubiertos. Uno de ellos no
puede tener requisito EARS porque no es verificable por software, y se declara
en vez de inventarle cobertura:

| Criterio | Cobertura | Estado |
|---|---|---|
| AC10 — "Gate humano: smoke en dev build de Android comparando lado a lado con el Figma, en tema claro Y oscuro, confirmando que ningún relleno cambió de color y que las etiquetas se leen. **No delegable a IA**" | **Sin R-id: no es automatizable.** Guion de la prueba en [[tasks]] §Cierre. El `leader` no puede marcar la feature `done` hasta que el humano lo cierre, aunque el `reviewer` haya aprobado | pendiente |
...
| 6 | R8 | ✔ |
| 7 | R9 | ✔ |
| 13 | R10 (menos `pet-switcher`, medido en 52 pt: [[design]] §D4) | ✔ |
| 19 | R6 | ✔ |
| 21 | R1 | ✔ |

Hallazgos 8-12, 14-18, 20 y 22-26: **fuera de alcance**, son la feature #62.
Hallazgo 18: cerrado por decisión humana del 2026-09-03, ver [[design]] §8.
