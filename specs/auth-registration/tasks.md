...
- [ ] Agregar dependencias `argon2` y `uuidv7` a `package.json`
  (`backend-pet-tracker/`)
- [ ] Crear `src/db/schema/users.schema.ts`,
  `src/db/schema/email-verification-tokens.schema.ts`,
  `src/db/schema/audit-log.schema.ts` (columnas exactas de
  `docs/data-model.md`)
- [ ] Actualizar `src/db/schema/index.ts`: quitar `schemaBootstrap`, agregar
  las 3 tablas nuevas (ver comentario de deprecación ya presente en ese
  archivo)
- [ ] `pnpm run db:generate` → migración versionada en `src/db/migrations/`
- [ ] Agregar `EMAIL_ENABLED=false` a `.env.example` (raíz del repo) y su
...
- [ ] Crear `src/audit/` (`audit-log.repository.ts` con interface
