...
| R8 | `src/workers/alerts-engine/alerts-engine-consumer.service.spec.ts::R8 (geofence-eval-full-batch #30): la alerta lleva el ts de la posición que cruzó` | rojo: `a37fe41 feat(geofence-eval-full-batch): add crossing timestamp tests (R8)`; verde: pendiente |
...
| 4 | Orden ascendente de `ts` y guard monotónico `previousUpdatedAtMs` respetado; un lote reentregado no reabre ni retrocede | R7 (orden) + R9a/R9b (guard) | orden cubierto por `312804c` → `13a65dd`; guard pendiente |
