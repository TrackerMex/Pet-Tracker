...
| R6 | `src/api/__tests__/pets.test.ts::R6: createPet publica el contrato exacto por kind`; `src/screens/add-pet/index.test.tsx::R6: alta de mascota`; `src/app/(tabs)/__tests__/layout.test.tsx::R10: las rutas de mascotas heredan SelectedPetProvider` | `99f0636` rojo → `f44f8dc` verde; fix smoke R10 `644a00c` rojo → `5bae7b0` verde |
...
| R10 | Regresión estructural: `src/app/(tabs)/__tests__/layout.test.tsx::R10: las rutas de mascotas heredan SelectedPetProvider`; smoke humano pendiente de repetirse | `644a00c` rojo → `5bae7b0` verde (hallazgo del smoke 2026-08-25); gate humano pendiente |
