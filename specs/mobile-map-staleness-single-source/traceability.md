...
| R8 | gate humano — smoke en dev build de Android, sin test automático | **cerrado** el 2026-09-21: casilla firmada en [[requirements]] §Aprobación → Cierre de R8, commit `ffeaf7af` |
...
| R10 *(E2)* | `src/__tests__/design-drift.test.ts::#94 R10: la antigüedad de la posición se lee en un solo sitio` | rojos `86594a29` por recuento (`feat(mobile-map-staleness): guard staleSeconds read inventory (R10)`) + `4c6dc3ab` por clave inesperada (`feat(mobile-map-staleness): probe staleSeconds read relocation (R10)`); verde `d04326b6` (`feat(mobile-map-staleness): enforce staleSeconds read inventory (R10)`) |
