...
| R15 | `src/__tests__/design-drift.test.ts`::`it('keeps the four Expo Router entrypoints thin')` con `app/(tabs)/home.tsx` dentro, **más** `HOME` entero en verde tras el movimiento, sin un solo assert debilitado | rojo `572d732` `feat(mobile-home-weekly-activity): test thin home route (R15)`; verde por completar |
