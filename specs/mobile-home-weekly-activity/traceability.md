...
| R12 | `CHART`::`R12: la tendencia sigue a la métrica y se calla sin base` (6 `it`: `+12,5`/`TrendUp`, `-8,3`/`TrendDown`, `0` sin icono, `null` sin fila, cambio de métrica, y `TABULAR_NUMS` sin color semántico) | rojo `c4250ff` `feat(mobile-home-weekly-activity): test metric trend row (R12)`; verde por completar |
