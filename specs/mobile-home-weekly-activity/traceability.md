...
| R14 | `HOME`::`R14: la Home monta la actividad semanal sin pedir nada nuevo` (4 `it`: la tarjeta con los siete días; el orden `summary-card` → `weekly-activity-card` → `last-position-card`; el recuento de llamadas a `getDailyActivity` sin cambio; skeleton y silencio en error para R14b) | rojo `23ae28a` `feat(mobile-home-weekly-activity): test home weekly card states (R14)`; verde por completar |
