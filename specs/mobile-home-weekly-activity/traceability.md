...
| R7 | `CHART`::`R7: la gráfica dibuja eje Y, rejilla y línea de media` (3 `it`: `formatYLabel` de longitud 4 exacta; `weekly-activity-average` con `TABULAR_NUMS`; media que ignora los `missing`) | rojo `e52f6e4` `feat(mobile-home-weekly-activity): test chart axis grid and average (R7)`; verde por completar |
