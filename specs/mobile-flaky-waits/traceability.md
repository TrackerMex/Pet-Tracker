...
| S2 · E1 (endurecimiento defensivo; C4 por invariancia) | `mobile-pet-tracker/src/screens/health/index.test.tsx` :: `R4: health resuelve la mascota seleccionada › keeps API order and selects the first pet by default` | `bcd8ba8a test(mobile-flaky-waits): align health query waits (R2)`; E1 en `790c58bd` |
...
| R2 · S4 | `mobile-pet-tracker/src/screens/map/index.test.tsx` :: `R7: ruta del día como polylines › R3 (android-map-never-ready): pasa un array vacío para un día sin viajes` | `9051eb77 test(mobile-flaky-waits): align map waits with assertions (R2)`; visibilidad restituida en `4477f0fc test(mobile-flaky-waits): restore map visibility assertion (R2)` |
