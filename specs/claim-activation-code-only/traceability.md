...
| `claim-device.dto.spec.ts::R4: recorta espacios del identificador` | El trim aplicaba sobre `esn` | `esn` ya no es una clave del schema; `result.data.esn` sería `undefined` y el test verde no probaría nada | El mismo trim sobre `activationCode` | rojo `740a0d4`; verde este commit (R1) |
