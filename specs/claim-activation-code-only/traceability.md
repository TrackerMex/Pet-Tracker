...
| R2 | `test/devices.e2e-spec.ts::R2 (claim-activation-code-only #26): imei, esn y serialNumber no reclaman nada` (`it.each` de los tres campos) y `::R1c (claim-activation-code-only #26): un imei ajeno junto al activationCode correcto se ignora` | implementación `cd33883` (R1); prueba R2 este commit (`test(claim-activation-code-only): prove legacy identifiers cannot claim (R2)`) |
