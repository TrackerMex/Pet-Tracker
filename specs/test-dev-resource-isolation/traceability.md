...
| R11 | `backend-pet-tracker/src/aws/resource-names-guard.spec.ts::R11: nadie importa los diez literales de nombre` | `backend-pet-tracker/src/aws/resource-names-guard.spec.ts` — **guarda de regresión, nace verde** | verde por excepción aprobada, con aserción anti-vacío: `a26ecf2 test(test-dev-resource-isolation): add approved green non-vacuous import guard (R11)` |
| R12 | `backend-pet-tracker/src/aws/resource-names-guard.spec.ts::R12: el stack CDK no importa la resolucion de sufijo` | `infra/` (**sin cambios** — evidencia: `git diff --name-only`) | verde por excepción aprobada: `9a18da5 test(test-dev-resource-isolation): add approved green cdk stack guard (R12)` |
| R13 | `backend-pet-tracker/src/aws/resource-names-guard.spec.ts::R13: verification.md documenta el recuento manual` | `docs/verification.md` | rojo: `e46ef81 test(test-dev-resource-isolation): add failing manual verification docs guard (R13)`; verde: `6897d8a docs(test-dev-resource-isolation): document manual queue verification (R13)` |
| R14 | sin test; evidencia: `git log --oneline` de `feature/28-test-dev-resource-isolation`, esta tabla completa y `progress/impl_test-dev-resource-isolation.md` con `./init.sh` exit 0 | `progress/impl_test-dev-resource-isolation.md`, `progress/imp.md` | `2beb12a docs(test-dev-resource-isolation): record implementation evidence (R14)` |
