...
| R5 — el día civil incluye mes y año (Enmienda E1; verificación; mutación `to.getDate() - from.getDate()`) | `#84 R5: el día civil incluye el mes y el año (Enmienda E1)` | pendiente | pendiente |
| R6 — umbrales de la píldora y del badge (Enmienda E1; verificación; mutación `<= 8` / `<= 11` en la pantalla) | `#84 R6: los umbrales de la píldora y del badge no se aflojan (Enmienda E1)` | pendiente | pendiente |
...
| `src/utils/reminder-dates.test.ts` · filas `zero`/`positive`/`negative` de `'returns a %s integer'` y su `const from` (Enmienda E1, H4) | instantes `Z` → componentes locales, mismos esperados | pendiente (rojo de R5) |
