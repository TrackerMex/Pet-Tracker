...
| R4 | `src/aws/media-bucket-aws-mode.spec.ts::R4: modo local ignora MEDIA_BUCKET_NAME` (resolvers + `.env.example` comentada); `progress/impl_media-bucket-aws-mode.md::Contención y modo local` (`env-drift`, lint, unit, e2e, `init.sh`, allowlist) | `a86844e test(media-bucket-aws-mode): define local containment in red (R4)` → `f577adc docs(media-bucket-aws-mode): document AWS bucket without local drift (R4)` → `8d77350 test(media-bucket-aws-mode): verify regression and containment (R4)` |
