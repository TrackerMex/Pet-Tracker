...
|---|---|---|
| R1 | `GRID`::`it('dibuja el rótulo y los tres tiles en orden')`: los tres `testID` presentes y en el orden del árbol, `quick-actions-title` con `Accesos rápidos` y la receta canónica de rótulo de sección, y ningún cuarto tile | `4552103` `feat(mobile-home-quick-actions): test quick actions grid (R1)` (rojo) → `5eb6c2c` `feat(mobile-home-quick-actions): add quick actions grid (R1)` (verde); corrección tras review: `9a4854e` (rojo, cuarto tile inline a `/pairing`) |
