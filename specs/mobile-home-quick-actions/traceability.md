...
|---|---|---|
| R1 | `GRID`::`it('dibuja el rótulo y los tres tiles en orden')`: los tres `testID` presentes y en el orden del árbol, `quick-actions-title` con `Accesos rápidos` y la receta canónica de rótulo de sección, y ningún cuarto tile | `4552103` `feat(mobile-home-quick-actions): test quick actions grid (R1)` (rojo) → `5eb6c2c` `feat(mobile-home-quick-actions): add quick actions grid (R1)` (verde); corrección tras review: `9a4854e` (rojo, cuarto tile inline a `/pairing`) → `7bf12d5` (verde) |
| R2 | `GRID`::`it('lleva cada tile a su ruta existente')` (`toHaveBeenNthCalledWith` con `'/weight-log'`, `'/add-reminder'` y `'/pets/pet-1/docs'` —el `id` de `makePet`, `HOME:94`—, una vez cada uno) + `GRID`::`it('no apunta a ninguna ruta inexistente')`, que cruza los destinos con los ficheros reales de `src/app/(tabs)/` leídos con `readdirSync` y fija que el `router.push` común no cubra `/weight-log` con `as Href` | `20355a6` `feat(mobile-home-quick-actions): test tile destinations (R2)` (rojo) → `48a0fe0` `feat(mobile-home-quick-actions): route each tile to its screen (R2)` (verde) + `167e505` `test(mobile-home-quick-actions): await tile presses (R2,R4)` (RNTL 14) + `7f967b3` `test(mobile-home-quick-actions): keep route inventory lint-clean (R2)`; corrección tras review: `da9a847` (rojo, cast común todavía presente) → `ba92380` (verde y `tsc --noEmit`) |
| R3 | `GRID`::`it('no dibuja ningún tile a una pestaña ni a un destino inexistente')`: exactamente tres hijos en la fila de tiles, y el **bloque `QUICK_ACTIONS`** del fuente sin `'/map'`, `'/health'`, `'/food'`, `'/trips'`, `'/reminders'`, `'/pairing'`, `'/pets/add'` ni `'/meal-schedule'`. La comprobación se acota al bloque de la constante porque la Home sigue navegando a `/map` (`:397`, `:411`) y a `/pairing` (`:362`) por su cuenta, y esos usos son legítimos | `d4c506e` `feat(mobile-home-quick-actions): test destinations earn their slot (R3)` (rojo) → `feb786f` `feat(mobile-home-quick-actions): keep tabs out of the grid (R3)` (verde); corrección tras review: `9a4854e` (rojo, cuarto tile inline a `/pairing`) → `7bf12d5` (verde) |
| R4 | `GRID`::`it('liga icono, etiqueta, color, tinta y destino de cada tile y de ninguno más')`, que para cada tile y con **`within(tile)`** exige **las cinco** dimensiones: icono por el doble de `reicon` (`icon-weight` / `icon-calendar-plus` / `icon-file-text`), etiqueta por `getByText`, clase `bg-category-violet` / `bg-category-amber` / `bg-category-blue` en el `className`, tinta resuelta `category-violet-strong` / `category-amber-strong` / `category-blue-strong`, y ruta al pulsar. **Nunca `getByTestId` global para el icono**: `icon-weight` sale dos veces en el árbol (celda de #69 y tile 1). Es el candado que mata **por separado** las cinco mutaciones de cruce | `b9b6926` `feat(mobile-home-quick-actions): test per-tile bindings (R4)` (rojo) → `50527a6` `feat(mobile-home-quick-actions): bind icon, label, colour and route per tile (R4)` (verde) + `167e505` `test(mobile-home-quick-actions): await tile presses (R2,R4)` (RNTL 14); corrección tras review: `8246d2e` (rojo, tinta fijada al índice 0) → `41838b5` (verde) |
| R5 | `GRID`::`it('resuelve el fondo y la tinta desde el mismo hueco')` (lectura de fuente: `CATEGORY_SLOTS[`, plantilla `` `category-${…}-strong` ``, ningún `bg-category-` a mano) + el cruce por tile de R4, que observa la tinta resuelta con `within(tile)`, + `src/__tests__/consistency-classnames.test.ts` **#64 R9** (`:372-433`) en verde y **sin tocar**, que es lo que prueba que la Home no escribió ninguna clase categórica. **La aserción de fuente es candado de cadena literal**, declarado como tal en [[requirements]] R13; la conducta la lleva R4 | `5c73799` `feat(mobile-home-quick-actions): test palette token wiring (R5)` (rojo) → `9f6b76c` `feat(mobile-home-quick-actions): take tile colours from the category palette (R5)` (verde); corrección tras review: `8246d2e` (rojo, tinta fijada al índice 0) → `41838b5` (verde) |
