...
| R2 | `GRID`::`it('lleva cada tile a su ruta existente')` (`toHaveBeenNthCalledWith` con `'/weight-log'`, `'/add-reminder'` y `'/pets/pet-1/docs'` —el `id` de `makePet`, `HOME:94`—, una vez cada uno) + `GRID`::`it('no apunta a ninguna ruta inexistente')`, que cruza los destinos con los ficheros reales de `src/app/(tabs)/` leídos con `readdirSync` | `20355a6` `feat(mobile-home-quick-actions): test tile destinations (R2)` (rojo) → verde pendiente |
