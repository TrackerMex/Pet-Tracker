...
| R14 | Los deltas de la tabla de [[requirements]] R14, verificados por el reviewer **rehaciendo cada `grep`**, no leyendo el informe del implementer. Fila 1: `src/providers/__tests__/language-provider.test.tsx:41`, `260 + 16 + 1` → `260 + 16 + 1 + 4` — el candado que **paró la implementación en #68 y en #69** por estar omitido. Esta feature **no reubica nada**: si aparece una reubicación, alguien está moviendo un fichero que no toca | `17c8c01` `feat(mobile-home-quick-actions): test catalog length delta (R14)` (rojo natural 281/277) → `bf18ed2` `feat(mobile-home-quick-actions): move locks by declared delta (R14)` (verde) |
