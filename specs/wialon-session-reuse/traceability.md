...
| R2 | `backend-pet-tracker/src/workers/poller.service.spec.ts::R2 (wialon-session-reuse #29): un ciclo del poller sobre N devices hace un solo token/login` | `backend-pet-tracker/src/integrations/wialon/wialon-http.client.ts` | `278018e` *(feat(wialon-session-reuse): cache sid por instancia en WialonHttpClient (R1))* |
...
| R7 | `backend-pet-tracker/src/integrations/wialon/wialon-http.client.spec.ts::R7 (wialon-session-reuse #29): el token no aparece en logs ni en errores` | `backend-pet-tracker/src/integrations/wialon/wialon-http.client.spec.ts` + `backend-pet-tracker/src/integrations/wialon/wialon.errors.ts` + `backend-pet-tracker/src/integrations/wialon/wialon-http.client.ts` | `2b28add` *(test(wialon-session-reuse): assert @nestjs/common solo por import real en wialon.errors (R7))*; `7a90e1f` *(feat(wialon-session-reuse): validate @nestjs/common import in R7 security check (R7))*; `7f0873f` *(feat(wialon-session-reuse): assert console spies before restoring mocks in R7 (R7))*; `5ef90f8` *(refactor(wialon-session-reuse): update wialon-http docs and import ordering (R7))* |
