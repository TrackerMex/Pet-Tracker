...
| R3 | `src/app/(tabs)/__tests__/weight-log.test.tsx::#90 R3: la fecha por defecto sale de la zona del perfil` (`it.each` Kiritimati/Pago_Pago) + los cuatro `it` editados con sufijo ` (#90 R3)` (tabla de abajo) | rojo `c8ee54b1` `test(mobile-owner-timezone-dates): the weight log default date must follow the profile zone (R3)` → verde pendiente |
...
|---|---|---|---|
| `weight-log.test.tsx:138` `R3 … › 'restaura los cuatro valores visibles tras el blur'` (`:158-160`) | #63 R3 | Reloj fijo + perfil Kiritimati; `toBe(localTodayIso())` → `toBe('2026-09-18')`; sufijo ` (#90 R3)`; fila de `specs/mobile-detail-screens-state-reset/traceability.md` actualizada | rojo `c8ee54b1` → verde pendiente |
| `weight-log.test.tsx:346` `R9 … › 'renders the inline form with the local date prefilled'` (`:355-357`) | R9 de weight-log | Ídem, dentro de `waitFor`; sufijo ` (#90 R3)` | rojo `c8ee54b1` → verde pendiente |
| `weight-log.test.tsx:377` `R9 … › 'submits all fields, clears them, and refetches the list (#72 R2)'` (`:406-408`) | #72 R2 (S7) | Ídem; la espera de `:401-403` se conserva; sufijo ` (#90 R3)`; fila de `specs/mobile-add-pet-photo-test-flake/traceability.md` actualizada | rojo `c8ee54b1` → verde pendiente |
| `weight-log.test.tsx:412` `R9 … › 'omits body condition when its field is blank'` (`:428`) | R9 de weight-log | `measuredAt: '2026-09-18'`; sufijo ` (#90 R3)` | rojo `c8ee54b1` → verde pendiente |
...
| `weight-log.test.tsx:84-89` `localTodayIso` (copia) | — | Renombrada `deviceTodayIso`; solo la usa R4 | rojo `c8ee54b1` → verde pendiente |
