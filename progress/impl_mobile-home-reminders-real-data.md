...
- **R15**: requisito de verificación. Las nueve mutaciones no versionadas se
  replantaron de una en una en producción durante el cierre; M1, M9, M10 y M13
  se auditaron en sus pares de commits rojos/verdes. Cada candado nominal cayó
  por el motivo previsto y cada restauración dejó `git diff --exit-code` vacío.
...
|---|---|---|
| M1 | Commit `97e06c0`: `localDayOf` cambió a `getUTC*`. `bun run test`, con `TZ` ausente, dejó 1 suite/1 test rojo y 67 suites/1082 tests verdes. Cayó exactamente `#85 R2` → `it('toma el día civil de los getters locales, nunca de los UTC')`: esperaba `2026-09-11`, recibió `2026-09-12`. `3d0ab78` completó el doble de padding para eliminar un `TypeError` secundario antes de registrar la evidencia definitiva. | Getters locales restaurados en `80595f9`; `format.test.ts` volvió verde y `git diff --exit-code` quedó vacío. |
| M2 | La Home pasó temporalmente `reminder.dueAt` crudo a `calendarDaysUntil` y `fmtDate`, sin `localDayOf`. Cayó exactamente `it('pinta fecha y contador reales para un dueAt con hora')`: `not.toHaveTextContent(/Invalid/)` recibió `Invalid Date`. | `localDayOf(reminder.dueAt)` restaurado sin commit; el `it` volvió verde y `git diff --exit-code` quedó vacío. |
| M3 | Se retiró temporalmente el filtro `status === 'scheduled'`. Cayó exactamente `#85 R3` → `it('descarta enviados, cancelados y pasados, y conserva el de hoy')`: esperaba `[rem-today, rem-next]` y recibió `[rem-today, rem-next, rem-sent]`; `rem-cancelled` queda cuarto y lo recorta el tope. | Filtro de estado restaurado sin commit; el `it` volvió verde y `git diff --exit-code` quedó vacío. |
| M4 | Se retiró temporalmente el filtro `calendarDaysUntil(localDayOf(dueAt), now) >= 0`. Cayó exactamente el mismo `it` de R3 que M3: esperaba `[rem-today, rem-next]` y recibió `[rem-past, rem-today, rem-next]`. El motivo y el id son distintos de M3. | Filtro de futuro restaurado sin commit; el `it` volvió verde y `git diff --exit-code` quedó vacío. |
| M5 | El corte inclusivo cambió temporalmente de `>= 0` a `> 0`. Cayó exactamente el `it` de R3 prescrito: esperaba `[rem-today, rem-next]` y recibió `[rem-next]`. | Corte inclusivo restaurado sin commit; el `it` volvió verde y `git diff --exit-code` quedó vacío. |
| M6 | Se retiró temporalmente `a.id.localeCompare(b.id)` del comparador. Cayó exactamente `it('desempata por id ascendente')`: esperaba `[rem-a, rem-z]` y recibió `[rem-z, rem-a]`. El sort estable conservó la fixture invertida. **Verde contrario**: si la fixture llegara ya `[rem-a, rem-z]`, esta mutación quedaría verde; por eso no se ordena. | Desempate por id restaurado sin commit; el `it` volvió verde y `git diff --exit-code` quedó vacío. |
| M7 | Se invirtió temporalmente el comparador primario a `b.dueAt.localeCompare(a.dueAt)`. Cayeron los dos candados nominales: `it('ordena por fecha ascendente')` esperaba `[rem-plus-1, rem-plus-3, rem-plus-6]` y recibió el orden inverso; `it('las ordena por fecha ascendente bajo la fila de la vacuna')` esperaba `[rem-b, rem-a, rem-c]` y recibió `[rem-c, rem-a, rem-b]`. | Comparador ascendente restaurado sin commit; ambos `it` volvieron verdes y `git diff --exit-code` quedó vacío. |
| M8 | El tope cambió temporalmente de `slice(0, 3)` a `slice(0, 4)`. Cayeron los dos candados nominales: `devuelve como mucho tres` recibió además `rem-plus-4`, y `corta en tres aunque haya cinco` recibió 5 hijos en vez de 4. | Tope de tres restaurado sin commit; ambos `it` volvieron verdes y `git diff --exit-code` quedó vacío. |
...
| M11 | Se añadió temporalmente al cuerpo `<View className="h-1.5 rounded-full bg-default" />` sin `testID`. Cayó exactamente `it('cuenta los hijos del cuerpo en tres escenarios')`: `children` esperaba longitud 1 y recibió 2. El candado cuenta hijos reales, no coincidencias de `testID`. | Hijo intruso retirado sin commit; el `it` volvió verde y `git diff --exit-code` quedó vacío. |
| M12 | Se cruzaron temporalmente `medication: Stethoscope` y `appointment: Pill`. Cayó exactamente `it('liga icono, superficie y tinta a su tipo')`: no pudo encontrar `icon-pill` dentro de `rem-b`. | Mapa exacto restaurado sin commit; el `it` volvió verde y `git diff --exit-code` quedó vacío. |
