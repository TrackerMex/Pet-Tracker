...
- `backend-pet-tracker/src/db/seed/vaccine-catalog.ts` — catálogo canónico de 4 vacunas dog y 3 cat, con upsert idempotente.
- `backend-pet-tracker/scripts/seed-vaccines.ts` — entrada standalone de `pnpm run seed:vaccines`.
- `backend-pet-tracker/src/modules/health/application/dto/vaccine.dto.ts` — validación zod estricta de catálogo, POST y PATCH.
...
- R2: test `src/db/seed/vaccine-catalog.spec.ts::R2: seed idempotente del catalogo de vacunas`, commit `aa12c8c`.
...
- R8: test e2e `R8: validacion y errores de catalogo`, commit `f09c84f`.
...
./init.sh
Build exitoso; 117/117 unit suites; 13/13 e2e suites;
