...
- **Queda del Bloque 1**: solo **#70 recordatorios**, sin especificar. Tiene **la mitad del diseno bloqueada**: `nextReminder` y `activitySummary` siguen hardcodeados a `null` en el mapper del perfil, asi que la barra de progreso de comidas no se puede construir. Su spec tiene que decir explicitamente que mitad entra.
