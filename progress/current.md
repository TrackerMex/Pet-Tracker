...
- **firma humana 2026-09-13**: commit `0a76562b` en la branch. Cambios sobre los defaults: **G2 = 120 s** (no 300) y **G7b = animación de pulso con Reanimated + reduced-motion dentro de esta feature**; el resto en default. Frontmatter de los 4 ficheros pasado a `approved`; `origin/main` (#124, init.sh sin FORCE_COLOR) mergeado en la branch (`bc29e5a0`). `spec_author` escribe las enmiendas E1 (120 s), E2 (pulso) y E3 (bloque E-#8 se copia firmado). Status de #73 → `in_progress`.
- **implementación 2026-09-14**: Codex tomó el handoff `progress/handoff_pet-online-pill.md`; baseline `./init.sh` verde. Ejecución TDD en orden R1→R10; R11 queda como gate humano.
