...
- Smoke (3er hallazgo): dark incompleto — Profile (Sun/Moon sin prop color), Health (Syringe stale al cambiar tema en caliente, tabs montadas no re-resuelven useThemeColor) y MapView sin customMapStyle dark. Handoff en progress/handoff_mobile-figma-polish-fix-dark-2.md.
- Codex inicia corrección post-smoke dark 2 (2026-08-23): baseline `./init.sh` exit 0 (móvil 27 suites/279 tests); cubrirá color explícito del toggle, re-resolución síncrona de tokens al cambiar `useUniwind().theme` en tabs montadas y `customMapStyle` night oficial de Google solo en dark. Sin push.
- Codex completa corrección post-smoke dark 2 (2026-08-23): commits `26c8f84`/`d28d406`/`b5bff10`; Profile e iconos de tabs usan colores reactivos sin `invalid`, y Map aplica el estilo night oficial solo en dark. Móvil 28 suites/284 tests, typecheck/lint y `./init.sh` verdes. Siguiente: repetir smoke dark en Android físico; R12 continúa abierto. Sin push.
