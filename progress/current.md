...
- **2026-09-03 — resuelto**: humano revirtió el `/pet` en `f18b4a7`; R1 verde (2/2); `git diff fb9db23 HEAD` sobre código = solo el fingerprint en `assetlinks.json`, así que el veredicto del reviewer sigue vigente. G1 y G2 verificados (traceability). Faltan G3 y G4, ambos a repetir con `hosting/` en la raíz.
